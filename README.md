...
<img align="left" width="400" height="500" src="https://user-images.githubusercontent.com/73979215/120214192-2451dd00-c202-11eb-89e6-2613890bb47a.png">jhqkc deluh wefdhecflu hiqew fhwhuq ewfulhwhjgvyuhgybjlsdjfjkhvfahhfuhcfwqhn weqino;winfdcinjfi nfcinofwionwefoe fwuefwubfuh fhufwhufwhu fweuhfcwuh fwucfwqf wqiofwcqionufio nfriofrhrehuerwhugrewhgei ngijngewijegrwijgerwij


 ## Conclusion

When analyzing the Mann-Kendall trend of fire pixels over the time period of 2001 to 2020 in Indonesia, we see that there is a trend that central and western portions of the country have an increasing trend of fires occurring. Riau and Jambi provinces in the western island, and central and western Kalimantan provinces, all have a significant amount of increasing pixels. For AOD, we see that there are various areas that have increasing trend values, mostly around the shore of the islands. Curiously, we see a significant amount of an increase in AOD tend values in Papua and West Papua provinces. When analyzing the fire-aod linear model residuals, we see a wide mixture of areas that are positive and negative, thus varying levels of under prediction and over-prediction; the under-prediction implies that there are factors that are contributing to higher AOD levels that are not addressed with this regression. Lastly, we see high R squared values for the provinces mentioned prior (especially Papua and West Papua provinces, which had a high concentration of high correlation values), but also we see higher correlation values in nearby Malaysia, suggesting that possibly there could be aerosols that end up blowing in the Indonesian region from fires occurring in Malaysia. Overall, we see an inconsistent response from the multiple regression procedure that cannot explain the amount of AOD in the region fully and would require additional research.

When analyzing the Mann-Kendall trend of fire pixels over the time period of 2001 to 2020 in the Pan-Amazonian region, we see that there is a wide mixture of increasing and decreasing trend values interspersed throughout the region.  We do see a cluster of increasing trend values in the Roraima state, a less-populated state of Brazil, existing in the Amazon rainforest region, surrounded by the Amazon river. Looking at the Mann-Kendall trend of AOD, we see a more clear pattern, with increasing trend values existing in the Amazon forest, near the Amazon river. The multiple regression procedure cannot reconcile the concentration of aerosols in the Pan-Amazon regions by the number of fires in the same areas. The high tau values of the linear model residuals indicate surprising amounts of aerosols and an increasing trend in aerosol concentration where fire observations are decreasing or not observed.
